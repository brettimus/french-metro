...
if (!health.ok)
  fail(
    `/healthz failed with ${health.status}. If this is an exe.dev login redirect, run: ssh exe.dev share set-public french-metro`,
  );
const body = (await health.json()) as { status?: string; commit?: string };
if (body.status !== "ok" || body.commit !== commit) {
  fail(
    `Health mismatch: expected commit ${commit}, got ${JSON.stringify(body)}`,
  );
}
...
if (!/<title>French Metro<\/title>/.test(html))
  fail("App page does not look like French Metro");
const asset = html.match(/src="(\/app-[A-Za-z0-9_-]+\.js)"/)?.[1];
if (!asset) fail("No app bundle found in app page");
if (asset.includes("app-HASH"))
  fail("Bundle substitution failed: template placeholder still present");
