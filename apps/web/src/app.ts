...
function showAbout() {
  const m = t(state.locale);
  about.innerHTML = `<button id="close-about" class="close-button" aria-label="${m.close}">×</button><div class="about-body"><h1 id="about-title">${m.aboutTitle}</h1><p>${m.aboutText}</p><p>${m.aboutSources}</p><p>${m.aboutLanguage}</p><p class="small-copy">${m.aboutPrivacy}</p></div>`;
  about.showModal();
  document.body.classList.add("dialog-open");
