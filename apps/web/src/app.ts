import { stations, lineSource, extensionSource, type Station } from './stations';
...
try { const saved: unknown = JSON.parse(localStorage.getItem('metro-stories-read') || '[]'); if (Array.isArray(saved)) visited = new Set(saved.filter((x): x is string => typeof x === 'string' && stations.some(s => s.id === x))); } catch { /* Reading progress is optional. */ }
const norm = (v: string) => v.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();
const count = () => { $('visited-count').textContent = `${visited.size} of 21 stories explored`; };
...
function renderStory(index: number) {
  ...
  $('share-story').onclick = async () => {
    try { await navigator.clipboard.writeText(location.href); $('share-story').textContent = 'Link copied ✓'; }
    catch { $('share-story').textContent = 'Copy the URL from your address bar'; }
  };
}
function openStory(index: number, replace = false, fromHash = false) {
  ...
  try { localStorage.setItem('metro-stories-read', JSON.stringify([...visited])); } catch { /* Browser storage may be disabled. */ }
  count();
  map.querySelectorAll('.station-node').forEach((n,i)=>n.classList.toggle('is-read',visited.has(stations[i]!.id)));
  if (!fromHash) history[replace || dialog.open ? 'replaceState' : 'pushState']({ station: true }, '', `#${stations[index]!.id}`);
  transition(()=> { renderStory(index); if (!dialog.open) dialog.showModal(); document.body.classList.add('dialog-open'); dialog.scrollTop=0; $('story-name').focus({preventScroll:true}); });
}
function closeStory() {
  history.replaceState(null, '', location.pathname + location.search);
  dialog.close();
}
dialog.addEventListener('close',()=>{ document.body.classList.remove('dialog-open'); if (location.hash) history.replaceState(null,'',location.pathname+location.search); opener?.focus({preventScroll:true}); });
dialog.addEventListener('cancel', e=> {e.preventDefault(); closeStory();});
dialog.addEventListener('click',e=>{if(e.target===dialog && (e.clientX<dialog.getBoundingClientRect().left || e.clientX>dialog.getBoundingClientRect().right)) closeStory();});
function hashChanged() { const index=stations.findIndex(s=>`#${s.id}`===location.hash); if(index>=0) openStory(index,true,true); else if(dialog.open) dialog.close(); }
addEventListener('popstate',hashChanged);
addEventListener('hashchange',hashChanged);
for (const container of [map,list]) {
  container.querySelectorAll<HTMLButtonElement>('[data-station]').forEach(btn=>{
    ...
    btn.addEventListener('keydown',e=>{ if (['ArrowDown','ArrowRight','ArrowUp','ArrowLeft'].includes(e.key)) { e.preventDefault(); const forward=e.key==='ArrowDown'||e.key==='ArrowRight'; const next=Math.max(0,Math.min(20,index+(forward?1:-1))); container.querySelector<HTMLButtonElement>(`[data-station="${next}"]`)?.focus(); } });
  });
}
function setView(isList: boolean) { listMode=isList; map.hidden=isList; list.hidden=!isList; $('map-view').setAttribute('aria-pressed',String(!isList)); $('list-view').setAttribute('aria-pressed',String(isList)); }
$('map-view').onclick=()=>{search.value=''; filter(); setView(false);};
$('list-view').onclick=()=>setView(true);
function filter() { const term=norm(search.value.trim()); if(term) setView(true); let matches=0; list.querySelectorAll<HTMLButtonElement>('[data-station]').forEach(btn=>{const s=stations[Number(btn.dataset.station)]!; const match=norm(s.name+' '+s.area).includes(term); btn.hidden=!match; if(match) matches++;}); $('no-results').hidden=matches!==0; }
search.oninput=filter;
search.addEventListener('keydown',e=>{if(e.key==='Enter'){const first=list.querySelector<HTMLButtonElement>('[data-station]:not([hidden])');first?.click();}});
$('preview-card').onclick=()=>openStory(selected);
$('start-button').onclick=()=>openStory(0);
$('surprise-button').onclick=()=>{const unread=stations.map((_,i)=>i).filter(i=>!visited.has(stations[i]!.id)); const choices=unread.length?unread:stations.map((_,i)=>i); openStory(choices[Math.floor(Math.random()*choices.length)]!);};
$('about-button').onclick=()=>{about.showModal();document.body.classList.add('dialog-open');};
$('close-about').onclick=()=>about.close();
about.onclose=()=>document.body.classList.remove('dialog-open');
window.addEventListener('keydown',e=>{if(e.key==='/' && !dialog.open && !about.open && !(e.target instanceof HTMLInputElement)){e.preventDefault();search.focus();} });
preview(6); count();
map.querySelectorAll('.station-node').forEach((n,i)=>n.classList.toggle('is-read',visited.has(stations[i]!.id)));
